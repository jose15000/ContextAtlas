...
// Mock EmbedQuery 
mock.module("../../MCP/functions/embedQuery.js", () => ({
...
describe("addReasoning", () => {
    it("adiciona exatamente 3 nós e 2 arestas ao grafo", async () => {
        const g = new Graph();
        await addReasoning(g, { reasoning: { prompt: "prompt", thoughtDescription: "thought", thoughtDetails: "decision", solution: "solution", toolCall: { tool: { name: "t", description: "d" }, result: "r" } } });
        expect(g.nodes.size).toBe(4); // prompt, thought, tool_call, implementation
        expect(g.edges.length).toBe(2);
    });
    it("cria nós com os tipos corretos", async () => {
        const g = new Graph();
        await addReasoning(g, { reasoning: { prompt: "p", thoughtDescription: "t", thoughtDetails: "decision", solution: "s", toolCall: { tool: { name: "t", description: "d" }, result: "r" } } });
    ...
    it("armazena os textos corretos nos nós", async () => {
        const g = new Graph();
        await addReasoning(g, { reasoning: { prompt: "meu prompt", thoughtDescription: "meu pensamento", thoughtDetails: "decision", solution: "minha solução", toolCall: { tool: { name: "t", description: "d" }, result: "r" } } });
    ...
    it("cada nó tem um timestamp ISO válido", async () => {
        ...
        for (const node of g.nodes.values()) {
            if (node.type === "tool_call")
                continue; // tool_call might not have timestamp in some implementations
    ...
    it("acumula múltiplas entradas no mesmo grafo", async () => {
        ...
        await addReasoning(g, data);
        await addReasoning(g, { reasoning: { ...data.reasoning, prompt: "p2" } });
...
describe("fluxo completo de persistência", () => {
    it("persiste e recarrega entradas de reasoning corretamente", async () => {
        ...
        await addReasoning(g1, { reasoning: { prompt: "Como funciona o BFS?", thoughtDescription: "O BFS usa uma fila e um Set de visitados", thoughtDetails: "decision", solution: "Implementei expandGraph com BFS iterativo", toolCall: { tool: { name: "t", description: "d" }, result: "r" } } });

...
import { indexClasses } from "./src/indexer/extractors/classes.js";
import { indexFunctions } from "./src/indexer/extractors/functions.js";
import { indexImports } from "./src/indexer/extractors/imports.js";
import { indexInterfaces } from "./src/indexer/extractors/interfaces.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const alwaysInProject = (_fp) => true;
// ---------------------------------------------------------------------------
// indexInterfaces
// ---------------------------------------------------------------------------
...
describe("indexFunctions", () => {
    it("adds a node for each named function", () => {
        const { sf, project } = makeProject(`export function hello() {} export function world() {}`);
    ...
    it("adds a DEFINES edge from the file to each function", () => {
        ...
        indexFunctions(sf, g, tc, alwaysInProject);
        const edges = g.getEdgesFrom("/file.ts");
        expect(edges.some((e) => e.type === "DEFINES" && e.to === "/file.ts#greet")).toBe(true);
    });
    it("stores the function name in node data", () => {
    ...
    it("does nothing when there are no functions", () => {
    ...
    it("adds CALLS edges when a function calls another function in the same file", () => {
        // Same-file resolution works reliably with the in-memory type checker.
        const { sf, project } = makeProject(`export function helper() {}
...
describe("indexClasses", () => {
    ...
    it("adds a DEFINES edge from the file to each class", () => {
        ...
        indexClasses(sf, g, tc, alwaysInProject);
        const edges = g.getEdgesFrom("/file.ts");
        expect(edges.some((e) => e.type === "DEFINES" && e.to === "/file.ts#Animal")).toBe(true);
    });
    it("adds nodes for each method inside a class", () => {
        const { sf, project } = makeProject(`class MyClass { greet() {} farewell() {} }`);
        const tc = project.getTypeChecker();
        const g = new Graph();
        g.addNode({ id: "/file.ts", type: "file", data: {} });
        indexClasses(sf, g, tc, alwaysInProject);
    ...
    it("adds DEFINES edges from class node to each method", () => {
        ...
        expect(classEdges.some((e) => e.type === "DEFINES" && e.to === "/file.ts#Svc.run")).toBe(true);
    });
    it("adds IMPLEMENTS edges when a class implements an interface", () => {
        const { sf, project } = makeProject(`interface Runnable {} class Worker implements Runnable {}`);
        const tc = project.getTypeChecker();
        const g = new Graph();
        g.addNode({ id: "/file.ts", type: "file", data: {} });
        indexClasses(sf, g, tc, alwaysInProject);
        const classEdges = g.getEdgesFrom("/file.ts#Worker");
        const implEdge = classEdges.find((e) => e.type === "IMPLEMENTS");
        expect(implEdge).toBeDefined();
        expect(implEdge?.to).toBe("Runnable");
    });
    it("stores class and method name metadata in node data", () => {
    ...
    it("does nothing when there are no classes", () => {
...
describe("indexImports", () => {
    it("adds an IMPORTS edge for each import declaration", () => {
        ...
        const sf = project.createSourceFile("/main.ts", `import { x } from "./utils";`);
        const g = new Graph();
        ...
        // Resolved path should point to the utils file
    ...
    it("adds one edge per import declaration", () => {
        ...
        const sf = project.createSourceFile("/main.ts", `import { a } from "./a";\nimport { b } from "./b";`);
        const g = new Graph();
        indexImports(sf, g);
        const edges = g.getEdgesFrom("/main.ts");

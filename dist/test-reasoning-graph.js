...
import os from "os";
// We need to control where the cache file lands.
// reasoningGraph.ts resolves the path as:
//   path.join(process.cwd(), ".codeatlas-reasoning.json")
// so we redirect process.cwd() to a temp directory for each test.
let tmpDir;
let cwdSpy;
// Dynamically import so the spy on process.cwd is in place before the
// module resolves its file path inside each function call.
import { addReasoning, getReasoningGraph } from "./src/graph/reasoning/reasoningGraph.js";

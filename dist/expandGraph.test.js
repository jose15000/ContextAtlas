...
import { expandGraph } from "./src/expansion/expandGraph.js";
...
describe("expandGraph", () => {
    ...
    it("with depth=1, visits A and B but not C/D/E", () => {
        ...
        const ids = nodes.map((n) => n.id);
        expect(ids).toContain("A");
        expect(ids).toContain("B"); // B is at level 1 (< depth 1 is false for B's children)
    ...
    it("returns empty nodes/edges for an unknown start id", () => {
        ...
        // node is not found (getNode returns undefined), but visited set has the id
        // edges array will be empty as there are no outgoing edges
